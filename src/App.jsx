...
function Dashboard({docs,issues,risks,actions,milestones}){
  ...
  return<div>
    ...
    <div className="grid-4"style={{marginBottom:16}}>
      ...
      <StatCard label="오픈 리스크"value={risks.filter(r=>r.status==='Open').length}color="var(--org)"/>
    ...
    <div className="tbl-wrap">
      <table className="tbl">
        <thead><tr>
          ...
          <th>담당</th><th>업무군</th><th>우선순위</th><th>마감일</th><th>비고</th>
        </tr></thead>
        <tbody>
          {filtered.length===0&&<tr><td colSpan={10}className="empty">조건에 맞는 문서가 없습니다</td></tr>}
...
/* ══════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════ */
function DocDetail({doc,onClose,onUpdate}){
  const[tab,setTab]=useState('info');
  const[form,setForm]=useState({...doc});
  const tabs=[{k:'info',l:'기본정보'},{k:'status',l:'작성현황'},{k:'wg',l:'업무군'},{k:'issues',l:'이슈/잔여과업'},{k:'notes',l:'메모/비고'}];
  ...
  return<div className="modal-overlay"onClick={onClose}><div className="modal"onClick={e=>e.stopPropagation()}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
      <div>
        ...
        <div style={{display:'flex',gap:8,marginTop:6}}><Badge text={form.status}/><Badge text={form.priority}/></div>
      </div>
...
export default function App(){
      ...
      try{
        ...
        if(!error){
          ...
          if(dbDocs&&dbDocs.length>0){
            ...
            setDocs(dbDocs.map(d=>({...d,work_groups:wgMap[d.id]||[]})));
          }else{
            // 시드 데이터 삽입
  ...
  const loadDocs=async()=>{if(!supabase)return;const{data:d}=await supabase.from('documents').select('*');const{data:w}=await supabase.from('document_work_groups').select('*');if(d){const wm={};(w||[]).forEach(x=>{if(!wm[x.document_id])wm[x.document_id]=[];wm[x.document_id].push(x.work_group);});setDocs(d.map(x=>({...x,work_groups:wm[x.id]||[]})));}};
  ...
  return<>
    ...
    {selectedDoc&&<DocDetail doc={selectedDoc}onClose={()=>setSelectedDoc(null)}onUpdate={updateDoc}/>}
  </>;
}
