...
function IssueList({issues,onUpdate,onAdd,onDelete}){
  ...
  return<div>
    ...
    {showNew&&<IssueEditModal issue={{id:`ISSUE-${String(issues.length+1).padStart(3,'0')}`,title:'',severity:'Medium',status:'Open',due_date:'',description:'',decision_needed:false,assignee:'',assignee_group:''}} onClose={()=>setShowNew(false)} onUpdate={i=>{onAdd(i);setShowNew(false);}} isNew/>}
  </div>;
}
...
function IssueEditModal({issue,onClose,onUpdate,isNew}){
  ...
  return<div className="modal-overlay"onClick={onClose}><div className="modal"onClick={e=>e.stopPropagation()}>
    ...
    <div className="form-row">
      <div className="form-group"><label>이슈 ID</label><input value={form.id}onChange={e=>setForm(p=>({...p,id:e.target.value}))}/></div>
...
function RiskList({risks,onUpdate,onAdd,onDelete}){
  ...
  return<div>
    ...
    {showNew&&<RiskEditModal risk={{id:`R-${String(risks.length+1).padStart(2,'0')}`,title:'',level:'Medium',status:'Open',description:'',mitigation:''}} onClose={()=>setShowNew(false)} onUpdate={r=>{onAdd(r);setShowNew(false);}} isNew/>}
  </div>;
}
...
function RiskEditModal({risk,onClose,onUpdate,isNew}){
  ...
  return<div className="modal-overlay"onClick={onClose}><div className="modal"onClick={e=>e.stopPropagation()}>
    ...
    <div className="form-row">
      <div className="form-group"><label>리스크 ID</label><input value={form.id}onChange={e=>setForm(p=>({...p,id:e.target.value}))}/></div>
      <div className="form-group"><label>제목</label><input value={form.title}onChange={e=>setForm(p=>({...p,title:e.target.value}))}/></div>
    </div>
    <div className="form-row">
...
function ActionList({actions,onUpdate,onAdd,onDelete}){
  ...
  return<div>
    ...
    {showNew&&<ActionEditModal action={{id:`A-${String(actions.length+1).padStart(2,'0')}`,title:'',assignee:'',due_date:'',priority:'Medium',status:'Open'}} onClose={()=>setShowNew(false)} onUpdate={a=>{onAdd(a);setShowNew(false);}} isNew/>}
  </div>;
}
...
function ActionEditModal({action,onClose,onUpdate,isNew}){
  ...
  return<div className="modal-overlay"onClick={onClose}><div className="modal"onClick={e=>e.stopPropagation()}>
    ...
    <div className="form-row">
      <div className="form-group"><label>액션 ID</label><input value={form.id}onChange={e=>setForm(p=>({...p,id:e.target.value}))}/></div>
...
export default function App(){
  ...
  const loadRisks=async()=>{if(!supabase)return;const{data}=await supabase.from('risks').select('*');if(data)setRisks(data);};
