...
export default function App() {
  ...
  async function submitPost() {
    if (!qnaOrgCat || !qnaOrgName || !qnaAuthor.trim() || !qnaTitle.trim() || !qnaContent.trim()) {
    ...
    try {
      const { error } = await supabase.from("qna_posts").insert({
        ...
        author: qnaAuthor.trim(), title: qnaTitle.trim(), content: qnaContent.trim()
      });
      if (error) throw error;
      setQnaOrgCat(""); setQnaOrgName(""); setQnaAuthor("");
  ...
  async function submitReply() {
    if (!replyOrgCat || !replyOrgName || !replyAuthor.trim() || !replyContent.trim()) {
      alert("모든 항목을 입력해주세요."); return;
    }
    ...
    try {
      const { error } = await supabase.from("qna_replies").insert({
        ...
        org_name: replyOrgName, author: replyAuthor.trim(), content: replyContent.trim()
      });
      if (error) throw error;
      setReplyOrgCat(""); setReplyOrgName(""); setReplyAuthor(""); setReplyContent("");
              ...
              {showNewPost && (
                <div style={{ background:"#0a1628", border:"1px solid #1e3a5f", borderRadius:10, padding:20, marginBottom:20 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:12 }}>
                    ...
                    <div>
                      <label style={{ fontSize:11, color:"#64748b", display:"block", marginBottom:4 }}>작성자 이름 *</label>
                      <input value={qnaAuthor} onChange={e => setQnaAuthor(e.target.value)} placeholder="이름 입력"
                    ...
                    return (
                      <div key={p.id} onClick={async () => { setSelectedPost(p); setShowNewPost(false); await loadReplies(p.id); }}
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6, flexWrap:"wrap" }}>
                          ...
                          <span style={{ fontSize:11, color:"#64748b" }}>· {p.author}</span>
            ...
            <>
                ...
                return (
                  <div style={{ background:"#0a1628", border:"1px solid #1e3a5f", borderRadius:10, padding:20, marginBottom:16 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10, flexWrap:"wrap" }}>
                      ...
                      <span style={{ fontSize:12, color:"#64748b" }}>· {selectedPost.author}</span>
                  ...
                  return (
                    <div key={r.id} style={{ background:"#060d1a", border:"1px solid #1e293b", borderLeft:`3px solid ${catCol.color}`, borderRadius:"0 8px 8px 0", padding:"12px 16px", marginBottom:8, marginLeft:16 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6, flexWrap:"wrap" }}>
                        ...
                        <span style={{ fontSize:11, color:"#64748b" }}>· {r.author}</span>
              ...
              <div style={{ background:"#0a1628", border:"1px solid #1e3a5f", borderRadius:10, padding:18 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:12 }}>
                  ...
                  <div>
                    <label style={{ fontSize:11, color:"#64748b", display:"block", marginBottom:4 }}>작성자 이름 *</label>
                    <input value={replyAuthor} onChange={e => setReplyAuthor(e.target.value)} placeholder="이름 입력"
                      style={{ ...inputStyle, width:"100%", boxSizing:"border-box", background:"#0f172a" }} />
                  </div>
                </div>
                <div style={{ marginBottom:12 }}>
